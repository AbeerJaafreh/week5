...
function calculateTax(num,tax) {
  // WRITE YOUR CODE UNDER THIS LINE 
  var pay=num*tax;
return "You got "+num+" JD from sales, you should pay "+pay+" JD for tax and you will have "+(num-pay)+" JD as net sales."
}

var numOfSale=window.prompt("Enter Number of Sales !");

var amountOfTax=window.prompt("Enter The Amount Of Tax  !");
calculateTax(numOfSale,amountOfTax);
...
function repeatChar(str,num) {
  ...
  while (num>0){
  	var repStr="";
  	while(num%2==0){
  		repStr+= str.toUpperCase()+" ";
  	}
  	while(num%2!==0){
  		repStr+= str.toLowerCase()+" ";
  	}
  	num--;
...
var num =window.prompt("Enter Number of time to repeat the Character !");
repeatChar(char,num);
...
function stringToCapital(str) {
  // WRITE YOUR CODE UNDER THIS LINE 
  var i=str.length()-1;
  return str.toUpperCase()+stringToCapital(str.length()-1);


}
var str="Abeer Jaafreh";
stringToCapital(str);
